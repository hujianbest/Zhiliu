# Interim keyword index is in-memory MiniSearch

**This is an implementation note, not the product retrieval decision.** ADR-0006 requires BM25 as the primary signal and ADR-0008 requires SQLite FTS5 (trigram, no compiled tokenizer). Ticket 09 still owns that rewrite. What shipped before the review round is recorded here so it is not mistaken for the lasting choice.
