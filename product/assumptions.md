...
- A-11 2026-08-30 [生效] AUTO：全文检索是全局对话框（可见「检索」+ Ctrl+K），不新增第四空间。主进程索引笔记 Markdown 与 EPUB 抽出正文；`indexStatus !== ready` 的来源在结果中标「部分索引」。网页文章与草稿类型先进入同一结果模型，当前语料是 EPUB 与笔记 — 默认理由: 票 08；语义检索与 Embedding ADR 属票 09。
- A-12 2026-08-30 [生效] AUTO：票 09 主套件用假 Embedding 适配器（与凭据假库同理，`ZHILIU_E2E=1`）；生产路径选可在 8GB 无独显运行的本地多语言模型并写入 ADR。GPU 只加速。语义检索走同一「检索」对话框的第二模式或自动混合，不新增入口。增量更新笔记/导入，禁止每查全库重建向量 — 默认理由: 票 09 与既有 E2E 缝。
