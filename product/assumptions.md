...
- A-22 2026-08-30 [生效] AUTO：一万笔记预算写死在 `docs/perf-budget.md` 与票 19；二十万块分块另设 seed ≤45s、检索 P95 ≤2s、RSS ≤1500MB。启动只等关键词索引，向量后台补齐。 — 默认理由: 票 19、ADR-0006。
- A-23 2026-08-30 [生效] AUTO：并发编辑用 `baseQuotation`/`baseThought` 做乐观比较；冲突写入 `*.conflict.md`，合并前不进检索。 — 默认理由: 票 20、ADR-0011。
- A-24 2026-08-30 [生效] AUTO：主题聚类按笔记中连续 2–4 个汉字分组；已有主题的笔记成员在后续组织中保留，未分配笔记才并入同名主题或新建。固定保护分组不被拆散；来源仍按三条思想笔记双向推导。 — 默认理由: 票 22、29、ADR-0002。
- A-25 2026-08-30 [生效] AUTO：后台 `runBackground` 在预算内先分析再聚类；思想线索自动写试写稿（试写+草稿、不进检索），书库发现只预填提案。崩溃报告默认关闭，开启后路径/汉字原文/sk- 凭据一律脱敏。 — 默认理由: 票 21、26、38。
- A-26 2026-08-30 [生效] AUTO：正式稿导出脚注列出稿件 spans 上的来源文档标题；无 sourceId 的段落不生成脚注。键盘路径上限写在 `docs/keyboard-paths.md`。 — 默认理由: 票 34、37、41。
- A-27 2026-08-30 [生效] AUTO：对话、正式稿生成与并列修订均走模型网关并写留痕；无模型或断网时这些动作可恢复失败，阅读/检索/手写仍可用。定稿时提出风格更新，须用户确认才写入档案。 — 默认理由: 票 23、27、36、39、42、第 3 轮代码门。
