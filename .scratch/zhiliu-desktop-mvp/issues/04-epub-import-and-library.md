...
- [x] 用户可以选择一个或多个本地 EPUB 文件导入
- [x] 导入时把原件复制进知识库目录，原始位置的文件不被改动（ADR-0001）
- [x] 来源文件与派生提取产物确实存在于知识库目录内，但被 Git 排除
- [x] 导入的来源文档立即出现在书库列表中，展示标题与可得的作者信息
- [x] 书库列表统一展示全部来源文档，并显示每个来源当前的索引状态（索引状态的取值与其变化断言由票 12 补充，本票只断言该位置存在且反映导入完成度）
- [x] 知识库目录被整体复制到别处后，来源文件与笔记关系都完整
